...
<template>
    ...
    <div class="relative flex justify-between items-start h-auto min-h-[8vh] max-h-screen bg-dots-darker bg-center bg-gray-100 dark:bg-dots-lighter dark:bg-gray-900 selection:bg-red-500 selection:text-white border-b dark:border-gray-800">
        ...
        <div v-if="canLogin" class="flex items-center top-0 right-0 p-6 mr-6 text-end z-10 gap-x-4">

            <!-- Dark Mode Toggler -->
            <label :class="darkMode ? 'bg-primary' : 'bg-stroke'"
                class="relative m-0 block h-7.5 w-14 rounded-full cursor-pointer" for="toggleThemeButton">
                <input type="checkbox" @change="darkMode = !darkMode"
                    @click.stop="toggleTheme" checked="darkMode" class="absolute top-0 m-0 h-full w-full cursor-pointer hidden" id="toggleThemeButton" />
            ...
            <Link v-if="$page.props.auth.user" :href="route('dashboard')" class="font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">儀錶板</Link>

            <template v-else>
                <Link :href="route('login')" class="font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">登入</Link>

                <Link v-if="canRegister" :href="route('register')" class="font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">注冊</Link>
            </template>
        </div>
